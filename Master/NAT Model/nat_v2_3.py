...
# Base seed - Ensemble için her model farklı seed kullanacak
BASE_SEED = 42

# Ensemble için seed'ler (her model farklı seed ile başlar)
ENSEMBLE_SEEDS = [42, 123, 456, 789]  # 4 model için 4 farklı seed

# İlk model için seed ayarla (ensemble döngüsünde değiştirilecek)
...
# Pretrained model URL'leri (alternatifler)
PRETRAINED_URLS = [
    "https://shi-labs.com/projects/nat/checkpoints/CLS/nat_tiny.pth",
    "https://huggingface.co/shi-labs/nat_tiny/resolve/main/nat_tiny.pth"
]
...
if USE_PRETRAINED:
    ...
    for PRETRAINED_URL in PRETRAINED_URLS:
        try:
            ...
            print(f"   🔄 Deneniyor: {PRETRAINED_URL}")
            ...
            # Model state_dict'ini al (farklı formatlar için)
            ...
            for k, v in pretrained_dict.items():
                # Key mapping (farklı isimlendirmeler için)
                model_key = k
                if k not in model_dict:
                    # Olası key varyasyonlarını dene
                    if k.startswith('module.'):
                        model_key = k[7:]  # 'module.' prefix'ini kaldır
                    elif 'backbone.' in k:
                        model_key = k.replace('backbone.', '')
                
                if model_key in model_dict and 'head' not in model_key:
            ...
            if len(filtered_dict) > 0:
                ...
                print(f"✅ Pretrained ağırlıklar yüklendi! ({len(filtered_dict)}/{len(model_dict)} katman)")
                ...
                os.remove(checkpoint_path)
                print(f"   ⚠️ Uyumlu ağırlık bulunamadı, bir sonraki URL deneniyor...")
    ...
    if not pretrained_loaded:
        print("⚠️ Pretrained model yüklenemedi (tüm URL'ler denendi)")
